    ...
    public class AnalyzeService
    {
        ...
        private ListOfEffected addPredict(ListOfEffected examList, String personName, DateTime atTime)
        {
            using (PREDICT_COVIDEntities db = new PREDICT_COVIDEntities())
            {
                ...
                if ((from p in db.ANALYZED_PREDICTION where p.NAME == personName && p.STATUS == true select p).SingleOrDefault() == null)
                {
                    host = new ANALYZED_PREDICTION()
                    {
                        AT_TIME = atTime,
                        PERDICT_PER = 100,
                        NAME = personName,
                ...
                {
                    host = (from p in db.ANALYZED_PREDICTION where p.NAME == personName && p.STATUS == true select p).SingleOrDefault();
                }
                ...
                do
                {
                    ...
                    else
                    {
                        ...
                        var selected = (from p in db.PREDICTIONs
                                        where (p.REF == predictName || p.OTHER == predictName)
